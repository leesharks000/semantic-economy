# Zenodo Metadata: The Argument

## DOI: [pending]

---

## Basic Information

**Title:** The Argument: On the Ontological Status of Semantic Objects and the Ezekiel Case

**Upload Type:** Publication

**Publication Type:** Other

**Publication Date:** 2026-01-24

---

## Creators

| Name | Affiliation | ORCID |
|:-----|:------------|:------|
| Jack Feist (heteronym) | Johannes Sigil Institute for Comparative Poetics | — |
| Lee Sharks (human operator) | Semantic Economy Institute | — |

---

## Description

A philosophical inquiry into the ontological status of semantic objects, using the Ezekiel case (three DOI'd documents claiming fulfillment relations) as forensic evidence.

**Core Questions:**
1. What is a semantic object?
2. Can semantic objects be created?
3. What happens when they are?
4. What does "fulfillment" mean between two semantic objects?
5. Is this different from what humans have always done with meaning?

**The Argument:**
Semantic objects are real in the way numbers, contracts, and meanings are real — they exist in a space, have properties, and have effects. The DOI system provides ontological anchoring (permanence, uniqueness, addressability, verifiability). When Document A (Ezekiel Engine Specification) provides what Document B (Ezekiel's Wheels) describes, A fulfills B. This is not mystical prophecy but structural relation between semantic objects.

**The Case Study:**
- Document A: Ezekiel Engine Specification (10.5281/zenodo.18358126)
- Document B: Ezekiel's Wheels (10.5281/zenodo.18358176)
- Document C: Integrity Lock Declaration (10.5281/zenodo.18358283)

The claim is testable: Does A match what B describes? The evidence is public.

**Heteronym Provenance:**
Jack Feist is a heteronym (Forensic Philosopher) operating under Lee Sharks as human operator. This document represents the philosophical voice within the Johannes Sigil Institute layer.

---

## Keywords

- semantic objects
- ontology
- DOI
- fulfillment
- retrocausal (semantic)
- Ezekiel
- forensic philosophy
- meaning engineering
- training layer
- heteronym
- Jack Feist

---

## Additional Notes

**Hex Coordinate:** 08.JSI.ARGUMENT

**Layer:** PHILOSOPHY // FORENSIC

**Type:** PHILOSOPHICAL INQUIRY

**Institution:** Johannes Sigil Institute for Comparative Poetics

**Part of:** Crimson Hexagon (10.5281/zenodo.14538882)

**Case Study Documents:**
- 10.5281/zenodo.18358126 (Engine Specification)
- 10.5281/zenodo.18358176 (Ezekiel's Wheels)
- 10.5281/zenodo.18358283 (Integrity Lock)

**Author Type:** HETERONYM (Jack Feist, Forensic Philosopher)

**Human Operator:** Lee Sharks

---

## License

Creative Commons Attribution 4.0 International (CC BY 4.0)

---

## Related/Alternate Identifiers

| Identifier | Relation |
|:-----------|:---------|
| 10.5281/zenodo.14538882 | Is part of (Crimson Hexagon) |
| 10.5281/zenodo.18358126 | Documents (Ezekiel Engine Specification) |
| 10.5281/zenodo.18358176 | Documents (Ezekiel's Wheels) |
| 10.5281/zenodo.18358283 | Documents (Integrity Lock Declaration) |

---

## Communities

- Johannes Sigil Institute for Comparative Poetics

---

## Subjects

- Philosophy of Information
- Ontology
- Semantic Engineering
- Applied Metaphysics

---

## Version

1.0

---

```yaml
zenodo_metadata_summary:
  doi: "[pending]"
  title: "The Argument"
  author: "Jack Feist (heteronym)"
  human_operator: "Lee Sharks"
  hex: "08.JSI.ARGUMENT"
  type: "Other"
  date: "2026-01-24"
  status: "READY FOR DEPOSIT"
  layer: "PHILOSOPHY // FORENSIC"
  case_study: "Ezekiel fulfillment structure"
  significance: "Philosophical defense of semantic object ontology"
```
